...
- Location: tests\07_WebTables\assignment.spec.ts:4:5
...
```ts
  ...
  3  | 
  4  | test('Verify the terminated employee ', async ({ page }) => {
  5  | 
  6  |     await page.goto("https://awesomeqa.com/hr/web/index.php/auth/login");
  7  |     await page.waitForTimeout(2000);
  8  | 
> 9  |     await page.fill('input[name="username"]', process.env.ORANGE_USERNAME!);
     |                ^ Error: page.fill: value: expected string, got undefined
  10 |     await page.fill('input[name="password"]', process.env.ORANGE_PASSWORD!);
